...
app.post('/api/ujzenekar', (req, res) => { 
    const ujzenekar = req.body; 
    if (!ujzenekar.nev || !ujzenekar.stilus_id || !ujzenekar.orszag || !ujzenekar.varos || !ujzenekar.aktiv_evek || !ujzenekar.tagok || !ujzenekar.legsikeresebb_album || !ujzenekar.kep_url) {
        return res.status(400).send('Missing required fields');
    }
    // Ellenőrizzük, hogy a stilus_id létezik-e
    db.query('SELECT id FROM stilusok WHERE id = ?', [ujzenekar.stilus_id], (err, results) => {
        if (err) {
            console.error(err);
    ...
    db.query('INSERT INTO `zenekarok` (`id`, `nev`, `stilus_id`, `orszag`, `varos`, `aktiv_evek`, `tagok`, `legsikeresebb_album`, `kep_url`) VALUES (?,?,?,?,?,?,?,?,?);', ujzenekar, (err, results) => { 
        if (err) { 
            console.error(err); 
            res.status(500).send('Error creating vegpont'); 
        } else { 
            res.status(201).json({ ...ujzenekar }); 
        } 
    }); 
}); 
