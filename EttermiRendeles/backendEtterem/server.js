...
app.get('/menuitems', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT menuitems.*, categories.name AS category_name FROM `menuitems` JOIN categories ON menuitems.category_id=categories.id ORDER BY category_id;');
        res.json(rows);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
